...
if not config["dev"]:
    if (
        ...
        and not config["openai"]["key"] == "gradio"
    ):
    ...
    HEADER = {"Authorization": f"Bearer {OPENAI_KEY}"}
else:
...
HTTP_Server = (
    "http://" + config["httpserver"]["host"] + ":" + str(config["httpserver"]["port"])
)
Model_Server = (
    "http://" + config["modelserver"]["host"] + ":" + str(config["modelserver"]["port"])
)
...
def send_request(data):
    ...
    if "openaikey" in data:
        HEADER = {"Authorization": f"Bearer {data['openaikey']}"}
...
def chitchat(messages, openaikey=None):
    data = {"model": LLM, "messages": messages, "openaikey": openaikey}
...
def huggingface_model_inference(model_id, data, task):
    ...
    if task in [
        ...
        response = requests.post(
            task_url, headers=HUGGINGFACE_HEADERS, json={"inputs": data["text"]}
        )
        return response.json()
    ...
    if task == "text-to-image":
        text = data["text"]
        response = requests.post(
            task_url, headers=HUGGINGFACE_HEADERS, json={"inputs": text}
        )
...
def run_task(input, command, results, openaikey=None):
    ...
    if task.endswith("-text-to-image") or task.endswith("-control"):
        if inference_mode != "huggingface":
            ...
            inference_result = {
                "error": f"service related to ControlNet is not available."
            }
    ...
    with multiprocessing.Manager() as manager:
        ...
        while True:
            ...
            for task in tasks:
                ...
                if (
                    len(list(set(dep).intersection(d.keys()))) == len(dep)
                    or dep[0] == -1
                ):
                    tasks.remove(task)
                    process = multiprocessing.Process(
                        target=run_task, args=(input, task, d, openaikey)
                    )
                    process.start()
                    processes.append(process)
...
def test():
    ...
    messages = [
        {
            "role": "user",
            "content": "Please generate a canny image based on /examples/savanna.jpg",
        },
        {
            "role": "assistant",
